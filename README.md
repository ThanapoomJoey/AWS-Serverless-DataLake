...
This project uses mock supermarket data to demonstrate building a serverless datalake on AWS cloud without physical servers. It leverages AWS services, assuming a team uploads data to S3 daily. The project covers initial setup and daily updates, focusing on the `supermarket_sales` table for daily updates (other tables remain static for simplicity). It includes manual steps via AWS UI, automation, and data modeling with fact and dimension tables.

The datalake uses S3 as storage, Glue for ETL and automation, Glue Crawler for DDL/table creation, Athena for querying, Step Functions for workflows, CloudWatch for logs, and EventBridge Scheduler for scheduling. It also involves partitioning and converting files to Parquet to save costs by reducing storage size and query scan costs. For example, converting a 5.9 MB CSV file to Parquet reduces it to 849.4 KB, saving **85.6%** of storage space.(Parquet reduces storage by ~X% and partitioning reduces query costs by ~Y%).
...
- See details in [S3_Bucket_Setup_for_Serverless_Datalake.md](import_data_to_S3/S3_Bucket_Setup_for_Serverless_Datalake.md)
...
- **Conclusion**: Converting CSV to Parquet reduces storage size by **85.66%**, saving significant space and costs in S3 (e.g., from ~$0.0146/month to ~$0.0021/month per file at $0.023/GB).
...
### Overall Impact
| Aspect            | Non-Partitioned       | Partitioned         | Reduction         |
|-------------------|-----------------------|---------------------|-------------------|
| File Size (CSV vs Parquet) | 636.7 MB (CSV) | 91.3 MB (Parquet) | 85.66%           |
| Data Scanned      | 99.64 MB             | 694.93 KB          | 99.3%            |
| Run Time          | 7.777 s              | 1.039 s            | 86.64%           |
